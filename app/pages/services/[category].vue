...
<template>
  <div>
    ...
    <section class="py-12">
      <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <UiCard v-for="service in categoryContent.services" :key="service.id" :to="service.link" :interactive="!!service.link" class="h-full">
    ...
    <!-- Responsible Offices -->
    <section v-if="categoryContent.offices.length > 0" class="py-12 bg-gray-50">
      <div class="container mx-auto px-4">
        <h2 class="text-xl font-bold text-gray-900 mb-6">
          Responsible Offices
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <UiCard v-for="office in categoryContent.offices.filter(office => office.hidden !== true)" :key="office.title" :to="office.link" padding="p-4" interactive class="flex items-center gap-4">
